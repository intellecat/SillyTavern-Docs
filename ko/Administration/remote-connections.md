...
icon: rss
order: -30
route: /ko/usage/remoteconnections/
label: 원격 연결
---
...
서버에서 사설 IP 범위(예: `192.168.x.x`, `10.x.x.x`)에 있는 주소로 나가는 HTTP 요청을 하는 것은 기본적으로 허용되지만, 화이트리스트 구성을 사용하여 특정 사설 주소로 액세스를 제한할 수 있습니다. 이것은 로컬 네트워크에서 실행 중인 사설 API에 ST가 접근하도록 허용하고 싶지만, ST가 로컬 네트워크의 다른 장치에 접근하는 것은 방지하고 싶을 때 권장됩니다.

#### "사설 주소"로 간주되는 것은?

* 루프백 주소: IPv4의 경우 `127.0.0.0/8`, IPv6의 경우 `::1/128`.
* IPv4 사설 주소 범위: A 클래스(`10.0.0.0/8`), B 클래스(`172.16.0.0/12`), C 클래스(`192.168.0.0/16`).
* 링크 로컬 주소: IPv4의 경우 `169.254.0.0/16`, IPv6의 경우 `fe80::/10`.
* 고유 로컬 주소: IPv6의 경우 `fc00::/7`.

#### 사설 주소 화이트리스트 토글

사설 주소 화이트리스트를 활성화하려면 SillyTavern 루트 디렉터리의 `config.yaml` 파일을 편집하십시오:
...
기본적으로 이는 서버에서 루프백 주소(`127.0.0.1` 및 `::1`)로만 요청을 허용합니다. 화이트리스트에 더 많은 사설 주소를 추가하려면 `privateAddressWhitelist.allowedRanges` 섹션에 포함하십시오:
...
이 예제는 서버에서 `192.168.x.x` 범위의 모든 주소와 루프백 주소로 요청을 허용하는 동시에, 다른 사설 IP 범위에 대한 액세스는 계속 차단합니다.
...
!!!warning
보안 및 개인 정보 보호를 위해 이미지를 빌드하는 경우 SSL 인증서를 Docker 이미지 안에 포함하지 마십시오. 대신 볼륨 마운트를 사용하여 런타임에 인증서를 제공하십시오.
!!!

Docker에서 SillyTavern을 실행할 때 SSL 인증서를 제공하는 권장 방법은 `/config` 볼륨 마운트에 인증서를 배치하는 것입니다. 이렇게 하면 컨테이너 이미지를 다시 빌드하지 않고도 인증서를 관리할 수 있습니다.

1. 인증서 파일(예: `privkey.pem` 및 `cert.pem`)을 컨테이너의 `/config`에 마운트된 로컬 config 디렉터리에 배치합니다.

2. 인증서를 참조하도록 `config.yaml`을 업데이트합니다:
...
3. 변경 사항을 적용하려면 Docker 컨테이너를 재시작합니다.
