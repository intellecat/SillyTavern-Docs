...
- `placeholder=string` - 입력 필드에 표시할 플레이스홀더 텍스트를 설정합니다.
- `tooltip=string` - 마우스를 올렸을 때 표시되는 툴팁을 설정합니다.
- `icon=string` - 팝업에 사용할 Font Awesome 아이콘 클래스를 설정합니다.
...
- `/swipe` — 마지막 캐릭터 메시지에 스와이프를 트리거합니다.
- `/regenerate` — 마지막 캐릭터 메시지를 재생성합니다.
- `/continue` — 마지막 메시지를 이어서 작성하려고 시도합니다.
...
생성된 텍스트는 파이프를 통해 다음 명령으로 전달되며 변수에 저장하거나 I/O 기능을 사용하여 표시할 수 있습니다:
...
1. `/char-create` — 명명된 인수로 제공된 데이터를 사용하여 새 캐릭터를 만듭니다.
2. `/char-update` — 명명된 인수로 제공된 데이터를 사용하여 현재 캐릭터를 업데이트합니다.
3. `/char-get` — 현재 캐릭터의 데이터를 JSON 객체로 검색하여 파이프로 전달합니다.
4. `/char-delete (name)` — 지정된 이름의 캐릭터를 삭제합니다.
5. `/char-duplicate (name)` — 지정된 이름의 캐릭터를 복제합니다.
6. `/tag-import (name)` — 캐릭터 카드 파일에서 태그를 가져옵니다.

## 로더 명령

로더 시스템은 시각적 피드백을 제공하거나 일시적으로 UI를 차단해야 하는 시간이 많이 걸리는 작업을 위한 재사용 가능한 오버레이를 제공합니다.

1. `/loader-show (text)` — 지정된 텍스트와 함께 로딩 오버레이를 표시합니다.
2. `/loader-hide` — 로딩 오버레이를 숨깁니다.
3. `/loader-wrap (closure)` — 로딩 오버레이를 표시하고, 제공된 클로저를 실행한 다음, 완료되면 오버레이를 숨깁니다.
4. `/loader-stop` — 로딩 오버레이를 중지하고 제거합니다.
